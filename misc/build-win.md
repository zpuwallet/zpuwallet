...
Test-Path "$($env:VCPKG_ROOT ?? 'C:\vcpkg')\installed\x64-windows-static-md\lib\libssl.lib"
```
...
- **`error ... -Werror,-Wnontrivial-memcall` in `rive_common` / harfbuzz** — newer
  **clang-cl** (the ClangCL toolset `rive_common` forces) treats `memcpy` on a
  non-trivially-copyable type as an error. The script sets
  `CCC_OVERRIDE_OPTIONS=^-Wno-error=nontrivial-memcall`, which the clang driver
  reads (and `cl.exe` ignores), demoting just that warning for the clang-cl plugin.
  If you build manually, `set CCC_OVERRIDE_OPTIONS=^-Wno-error=nontrivial-memcall`.
