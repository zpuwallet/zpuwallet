...
.PARAMETER Msys2Root
    Path to the MSYS2 installation (the directory that CONTAINS ucrt64\). If
    omitted, it is auto-resolved in this order: $env:MSYS2_ROOT -> C:\msys64 ->
    the InstallLocation recorded by the MSYS2 installer in the registry. Pass
    this only to override that search.
...
.PARAMETER FlutterVersion
    ...
    the build, mirroring zwallet's build-msys2.ps1. Default: 3.44.2. Set to ""
...
# Resolve the MSYS2 root directory (the folder that CONTAINS ucrt64\). Priority:
#   1. an explicit -Msys2Root argument (caller override),
#   2. $env:MSYS2_ROOT (a convention some setups export),
#   3. C:\msys64 (the conventional install location),
#   4. the InstallLocation the MSYS2 installer records in the registry (HKLM/HKCU,
#      32- and 64-bit views), so a non-default install is still found.
# Returns the first candidate that actually has ucrt64\bin\gcc.exe, else the
# first non-empty candidate so the caller emits a consistent "not found" error.
function Resolve-Msys2Root($explicit) {
    ...
    # The MSYS2 installer registers an uninstall entry; its InstallLocation points
    # at the MSYS2 root. Probe the usual uninstall keys across hives/views.
    $regKeys = @(
        "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\MSYS2",
        "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\MSYS2",
        "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\MSYS2"
    )
    # NOTE: under Set-StrictMode, reading a missing property (.InstallLocation)
    # throws even with -ErrorAction SilentlyContinue (that only suppresses the
    # cmdlet's own errors, not the strict-mode property access). So fetch the key
    # object, then guard the property with Get-Member before reading it.
    foreach ($k in $regKeys) {
        $props = Get-ItemProperty -Path $k -ErrorAction SilentlyContinue
        if ($props -and ($props.PSObject.Properties.Name -contains "InstallLocation")) {
            $loc = $props.InstallLocation
            if (-not [string]::IsNullOrWhiteSpace($loc)) { $candidates.Add($loc) }
        }
    }

    foreach ($c in $candidates) {
        if (Test-Path (Join-Path $c "ucrt64\bin\gcc.exe")) { return $c }
    }
    return $candidates[0]
}
...
# Resolve the MSYS2 root (-Msys2Root / MSYS2_ROOT / C:\msys64 / registry), then
# the UCRT64 sub-tree of it (gcc + OpenSSL live here).
...
if (-not (Test-Path (Join-Path $Ucrt64Bin "gcc.exe"))) {
    throw "Could not find UCRT64 gcc at '$Ucrt64Bin\gcc.exe'. Searched -Msys2Root, `$env:MSYS2_ROOT, C:\msys64, and the MSYS2 registry entry. Install the MSYS2 'mingw-w64-ucrt-x86_64-gcc' package, or pass -Msys2Root <path>."
}
...
# pass it straight to --dart-define.
...
# (e.g. "6.18.11-rc.3" has none), so split defensively instead of indexing [1]
# blindly - a missing '+' part would otherwise throw "Index was outside the
# bounds of the array." Default the build number to 0 when absent.
...
# ----------------------------------------------------------------------------
# 4b. Silence noisy THIRD-PARTY plugin warnings during the Flutter C++ build
# ----------------------------------------------------------------------------
# Only the Rust rlz.dll uses the GNU/UCRT64 toolchain (Section 2). The Flutter
# Windows build below still compiles the C++ plugins with MSVC + clang-cl, so the
# same newer-toolset warning flood as build-win.ps1 applies. We suppress those
# specific diagnostics process-scoped (nothing is written into the plugin trees),
# via env vars the compilers read:
#
...
#    /wdNNNN in CL disables them for cl.exe targets (pure noise).
...
#    '^' means "APPEND this argument", so the flag lands at the END of the command
#    line - AFTER the plugin's own /WX (-Werror) - letting us fully SILENCE it
#    (the same flag in windows/CMakeLists.txt gets overridden by a later /WX; see
#    commit 67856063, which is why that file keeps the weaker -Wno-error= form).
...
if ([string]::IsNullOrEmpty($env:CL)) {
    $env:CL = "$coroutineDefine $msvcQuietWarnings"
} else {
