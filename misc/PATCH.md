...
- **UI Fix**
  ...
  - [Tutorials skipped by default](#tutorials-skipped-by-default)
- **Settings New**
...
- **FRB bindings:** `wire__crate__api__account__generate_prev_dindex_impl` +
  ...
  `generate_prev_dindex` (and changing the `open_database` signature) shifted
  the ids of every later function — the whole `funcId` table was renumbered by
  the regen (hence the large `frb_generated.*` diff), so this is a full codegen
...
- `lib/store.dart` — `SyncStateAccount.build` (the per-account progress card):
  when the account is part of the active sync, the displayed height is now
  driven by the **live synchronizer progress** (`ss.height`) so the card climbs
  toward the chain tip in real time, instead of staying frozen at
  `account.height` (which is only refreshed from the DB after the whole sync
  completes):
  - `start: account.height` (was `max(ss.start, account.height)`) — the
    account's stored height is the sync start, so the progress bar renders
    correctly.
  - `time: ss.time != 0 ? ss.time : account.time` (was
    `max(ss.time, account.time)`) — prefer the live timestamp, falling back to
    the stored one before the first progress event.
- Incidental: the (already unused upstream) `package:collection/collection.dart`
  import in `lib/store.dart` was removed to keep `flutter analyze` clean.

## Tutorials skipped by default

**(Dart-only)**

- `lib/utils.dart` — the showcase tutorials no longer auto-play on a fresh
  install; they only run after the user explicitly asks for them via
  **Settings → "Show Tutorials again"**.
  - `tutorialHelper`: the default when a per-tutorial flag (`tutMain0`,
    `tutSend0`, …) is unset changed from `true` (play) to
    `?? await prefs.getBool("tutReset") ?? false` (skip unless tutorials were
    explicitly re-enabled).
  - `resetTutorial`: additionally sets a new `tutReset` pref to `true` before
    clearing the individual flags, which is what re-arms `tutorialHelper`'s
    fallback. Once a tutorial plays, its own flag is stored `false` again as
    before, so each one still shows only once per reset.
...
- `lib/store.dart` — two default values changed in `AppSettingsNotifier.build`.
  ...
  - **`get_fx` pref default `false` → `true`:** "Auto Fetch Market Price" is on
    by default, so balances show a fiat value out of the box. Note this makes
    the app poll the CoinGecko API (every minute, see `PriceNotifier`) without
    an explicit opt-in — privacy-conscious users can turn it off in Settings
    (or use Tor/offline mode as before).
  - **`sync_interval` prop default `"30"` → `"1"`:** auto-sync runs every
    minute instead of every 30 minutes, keeping balances/confirmation counts
    (see **UI New → Confirmation count**) close to the chain tip by default.
