...
- **Wallet Fix**
  - [Chunked `GetBlockRange` sync windows](#chunked-getblockrange-sync-windows)
...
- **Settings New**
  ...
  - [Blocks per Sync Window field](#blocks-per-sync-window-field)
...
> **Rebase note (6.20.0 → 6.21.0):** upstream added a **rhai-based plugin
> system** for memo parsing (new `plugin_manager.dart`, `plugin_memo_view.dart`,
> `vault_account_picker.dart`, `rust/src/plugin/*`, `lib/src/rust/api/plugin*`,
> a `plugins` DB table, and `rhai`/`zip` Rust deps). It is orthogonal to our
> features and all hooks (`initPlugins()` in `main.dart`, `PluginMemoView` in
> `account.dart`/`tx_view.dart`, the Plugin Manager row in `settings.dart`, the
> `/settings/plugins` route, and the `pluginList`/`pluginMemoSections`
> providers in `store.dart`) were kept alongside ours. One patch was
> **simplified**: upstream refactored `accounts.dart` to render the empty state
> via an `EditableList` `emptyBuilder:` callback and extracted the AppBar actions
> into `_buildAppBarActions()`. Because the list's AppBar is now always built
> (even when empty), our previous standalone-`Scaffold` empty-state workaround
> (added so the Switch Network button stayed reachable with no accounts) is no
> longer needed and was dropped; the **globe Switch Network button** now lives in
> `_buildAppBarActions()` and the per-network `title:` (`networkTitle(...)`) is
> reachable in both the populated and empty states.
...
## Chunked `GetBlockRange` sync windows

Requires FRB regen + `RUSTFLAGS` rebuild (see Build / regen notes).

**Files:** `rust/src/sync.rs`, `rust/src/api/sync.rs`,
`rust/src/graphql/frost.rs`, `rust/src/graphql/mutation.rs`, `lib/store.dart`,
`lib/settings.dart` (+ regenerated `frb_generated.*`, `*.freezed.dart`,
`*.g.dart`)

- **Problem:** the shielded sync opened a **single** `GetBlockRange` stream over
  the entire `start..=end` range. One long-lived stream is fragile — any
  proxy/idle/reorg drop (or a server-imposed stream deadline on public nodes
  like `zec.rocks`) tears down the whole range and restarts it from the
  beginning.
- **`rust/src/sync.rs` — windowed loop:** `shielded_sync` now walks
  `start..=end` in fixed-size block windows, opening a **fresh** `GetBlockRange`
  per window. The original single-range body was extracted verbatim into a new
  `shielded_sync_window(...)` helper; `shielded_sync` loops over it, advancing
  `window_start = window_end + 1`. Each window keeps its stream short-lived, so a
  dropped stream costs at most one window instead of the whole range.
- **Resumable by construction:** the sync was already resumable at any block
  boundary — `Synchronizer::new` bootstraps each run from the witnesses
  persisted at `window_start - 1`, and the existing `Checkpoint` / `Witness` /
  `BlockHeader` messages commit per chunk (always including the window's last
  block). Chunking just exploits this: each window resumes from the prior
  window's committed state. The window size (gRPC stream granularity) is
  orthogonal to `actions_per_sync` (DB-commit granularity).
- **Cancellation:** the cancel handle changed from a consumed
  `broadcast::Receiver` to `&broadcast::Sender`, so each window can `.subscribe()`
  a fresh receiver. A persistent receiver (subscribed once before the loop, so it
  can't miss a mid-window signal) is polled **between** windows to stop a
  cancelled sync cleanly at a committed boundary.
- **Default & callers:** new `DEFAULT_BLOCK_CHUNK_SIZE = 500`. A
  `block_chunk_size: u32` parameter was threaded through `shielded_sync`,
  `synchronize_impl`, and the FRB `synchronize` entrypoint (`api/sync.rs`); the
  three internal `synchronize_impl` callers (`graphql/frost.rs`,
  `graphql/mutation.rs` ×2) pass `DEFAULT_BLOCK_CHUNK_SIZE`.
- **Dart wiring (`store.dart`):** the Flutter sync call passes
  `blockChunkSize: int.parse(settings.blockChunkSize)` (see **Settings New →
  Blocks per Sync Window field** for the setting itself).

> See also **Settings New → Blocks per Sync Window field** (user-editable size).
...
## Blocks per Sync Window field

Requires FRB regen + `RUSTFLAGS` rebuild (the backing param crosses the FRB
boundary — see **Wallet Fix → Chunked `GetBlockRange` sync windows**).

**Files:** `lib/settings.dart`, `lib/store.dart` (+ regenerated
`*.freezed.dart` / `*.g.dart`)

- **`lib/settings.dart`:** a new **"Blocks per Sync Window"** integer field
  (next to "Actions per Sync"), with an `onChangedBlockChunkSize` handler and an
  integer-only validator, persisted to the `block_chunk_size` DB prop. Tooltip
  explains that smaller windows keep each network stream short-lived and make
  sync resumable.
- **`lib/store.dart`:** `AppSettings` gains a `blockChunkSize` field, loaded from
  the `block_chunk_size` prop (default `"500"`) and passed to the `synchronize`
  Rust call. Controls the per-window block count for the chunked sync.
