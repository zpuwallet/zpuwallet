...
.DESCRIPTION
    ...
    OpenSSL is installed by this script in vcpkg MANIFEST mode: the repo-root
    vcpkg.json declares `openssl` as a dependency, and the script runs
    `vcpkg install` into a project-local tree (build\vcpkg_installed) rather than
    the shared <VcpkgRoot>\installed. vcpkg uses its own bundled Perl for that
    one-time build, so you never install Perl yourself; repeat runs restore from
    vcpkg's binary cache.

    Concretely:
      1. Install openssl:x64-windows-static-md via vcpkg manifest mode into
...
# ----------------------------------------------------------------------------
# 2. Install vcpkg OpenSSL (MSVC ABI) in MANIFEST mode -> build\vcpkg_installed
# ----------------------------------------------------------------------------
...
#
# MANIFEST mode (not classic): the repo-root vcpkg.json declares `openssl` as a
# dependency, and `vcpkg install` materializes it into a project-local tree
# instead of the shared <VcpkgRoot>\installed. We pin that tree to
# build\vcpkg_installed via --x-install-root, so the dependency lives next to the
# Flutter build output (and is wiped by -Clean along with the rest of build\).
# vcpkg restores from its binary cache on repeat runs, so this is fast after the
# first (~10-20 min) build of OpenSSL.
Write-Step "Installing vcpkg OpenSSL ($OpensslTriplet) in manifest mode"
$VcpkgManifest = Join-Path $RepoRoot "vcpkg.json"
if (-not (Test-Path $VcpkgManifest)) {
    throw "vcpkg manifest not found at '$VcpkgManifest'. It declares the OpenSSL dependency for the manifest-mode install."
}
$VcpkgInstallRoot = Join-Path $RepoRoot "build\vcpkg_installed"
New-Item -ItemType Directory -Force -Path (Join-Path $RepoRoot "build") | Out-Null
Write-Info "Manifest: $VcpkgManifest"
Write-Info "Install root: $VcpkgInstallRoot"
# Manifest-mode install reads vcpkg.json from --x-manifest-root and writes the
# resolved packages under --x-install-root\<triplet>. vcpkg uses its OWN bundled
# Perl to build OpenSSL, so no Perl/NASM is needed on the host.
Invoke-Checked $VcpkgExe @(
    "install",
    "--triplet=$OpensslTriplet",
    "--x-manifest-root=$RepoRoot",
    "--x-install-root=$VcpkgInstallRoot"
)

$OpensslDir = Join-Path $VcpkgInstallRoot $OpensslTriplet
...
if (-not ((Test-Path $OpensslHdr) -and (Test-Path $LibSsl) -and (Test-Path $LibCrypto))) {
    throw @"
OpenSSL ($OpensslTriplet) was not produced by the manifest-mode install under:
    $OpensslDir
Check the vcpkg output above for the failure. (Do NOT use the x64-windows-static
triplet - its /MT CRT mismatches Rust-msvc and causes LNK2038.)
"@
}
