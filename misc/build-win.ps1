...
# plugins more strictly and break two THIRD-PARTY plugin sources we don't own.
# We suppress those two specific diagnostics process-scoped (nothing is written
# into the plugin trees), via env vars the compilers read before their args:
#
...
#  * rive_common's bundled harfbuzz hits clang-cl's -Werror,-Wnontrivial-memcall
#    (memcpy on a non-trivially-copyable type). CCC_OVERRIDE_OPTIONS is read ONLY
#    by the clang driver (clang-cl) - cl.exe never sees it - so we demote just
#    that warning for the ClangCL-toolset plugin without touching MSVC targets.
#    The leading '^' means "append this argument" (clang's override syntax).
$coroutineDefine = "/D_SILENCE_EXPERIMENTAL_COROUTINE_DEPRECATION_WARNINGS"
if ([string]::IsNullOrEmpty($env:CL)) {
    $env:CL = $coroutineDefine
} elseif ($env:CL -notlike "*_SILENCE_EXPERIMENTAL_COROUTINE_DEPRECATION_WARNINGS*") {
    $env:CL = "$($env:CL) $coroutineDefine"
}
$env:CCC_OVERRIDE_OPTIONS = "^-Wno-error=nontrivial-memcall"
